	...
	class CommentResponseBuilder implements ResponseBuilder{
		public function build($request){
			...
			if(isset($_GET['productId']) && is_numeric($_GET['productId'])){
				...
				if(!empty($result)) return json_encode(array('status'=>true,'code'=>SUCCEED,'comments'=>$result),true);
				else return LocalApiResponseGetter::createResponse('false',E_NO_COMMENT,'NO COMMENT');
			}else if(isset($_GET['commentId']) && is_numeric($_GET['commentId'])){
				$result=$adapter->getComment($_GET['commentId']);
				if($result!==null) return json_encode(array('status'=>true,'code'=>SUCCEED,'comment'=>$result),true);
				else return LocalApiResponseGetter::createResponse('false',E_NO_COMMENT,'NO COMMENT');
			}else{
